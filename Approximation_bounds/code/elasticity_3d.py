...
W = 0.2
N = 10
mesh = BoxMesh(int(L / W) * N, N, N, L, W, W, hexahedral=True)
...
bcs = [DirichletBC(V, Constant((0.0, 0.0, 0.0)), (1))]
...
def sigma(u):
    return lambda_ * tr(epsilon(u)) * Id + 2 * mu * epsilon(u)
...
Mn = np.empty((3 * u.dat.data.shape[0], 0))
Rn = np.empty((3 * u.dat.data.shape[0], 0))
for k in range(num_eigenvalues):
    rhs.assign(eigenfunctions[k])
    solve(F == LHS, u, bcs=bcs, solver_parameters=params)
    Mn = np.hstack((Mn, np.reshape(u.dat.data, (-1, 1), order="C")))
    Rn = np.hstack((Rn, eigenvalues[k] * np.reshape(u.dat.data, (-1, 1), order="C")))
...
# Save data
np.savetxt(
    "elasticity_3d_result.csv",
    np.vstack((range(1, num_eigenvalues), np.array(norm_E), np.array(RHS))).T,
    delimiter=",",
)
np.savetxt(
    "elasticity_3d_Mn.csv",
    np.vstack((range(1, num_eigenvalues + 1), np.array(norm_ALP))).T,
    delimiter=",",
)
...
plt.savefig("fig_eig_%d_lmbda_%.2e_N_%d.png" % (num_eigenvalues, float(lambda_), N))
